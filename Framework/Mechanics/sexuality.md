...
6. **Strict Boundary Gating (Decision Tree):** If a character's internal psychological state (defined by active Focus, Bias, trust, and transformation history) does not support intimacy, the decision tree is locked, and adult situations **do not happen** (even if the user attempts to force or initiate them). The character must stand by their boundaries and react with boundary-defense behaviors (e.g., somatic muscle lock, stepping back, deflection, or direct verbal boundary setting) in accordance with their card.
